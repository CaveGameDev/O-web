...
Scene_OmoriQuest.prototype.onQuestListOk = function() {
  ...
  // Get Messages. Older saves can contain a removed quest or message index;
  // treat that entry as empty instead of passing undefined to the language
  // processor.
...
Window_OmoriQuestList.prototype.selectedQuestMessages = function(index = this._index) {
  // Get Quest and its current localized message list.
  var quest = this._questList[index];
  var questData = quest && $dataQuests && $dataQuests.quests && $dataQuests.quests[quest.id];
  if (!questData || !questData.text) { return []; }
  var messages = questData.text[quest.messageIndex];
  // Keep only valid message codes that still exist in the loaded language
  // file. This prevents removed entries in an old save from reaching
  // Game_Message.showLanguageMessage (and avoids showing a fallback error).
  var languageData = LanguageManager.languageData && LanguageManager.languageData();
  var textData = languageData && languageData.text;
  return Array.isArray(messages) ? messages.filter(function(code) {
    if (typeof code !== 'string' || code.length === 0) { return false; }
    var separator = code.indexOf('.');
    if (separator <= 0 || separator >= code.length - 1) { return false; }
    var fileData = textData && textData[code.slice(0, separator)];
    return !!(fileData && fileData[code.slice(separator + 1)] !== undefined);
  }) : [];
};
//=============================================================================
// * Make Command List
...
Window_OmoriQuestList.prototype.makeCommandList = function() {
  ...
  for (var i = 0; i < this._questList.length; i++) {
    ...
    var quest = this._questList[i];
    // Get Data
    const questNames = LanguageManager.getMessageData("XX_BLUE.Quest_Names") || {};
    const loc = questNames[quest.id]
