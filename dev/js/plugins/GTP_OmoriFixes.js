...
(function($) {
	...
	//###############################################################################
	//
	// BROWSER FIX: Font vertical positioning adjustment
	// The M+ font renders slightly higher in browser canvas vs NW.js.
	// Increase the cap-height ratio from 0.7 to 0.8 to shift text down ~1-2px.
	...
	//###############################################################################
	//
	// BROWSER FIX: Define missing voidFilter references
	// VoidFilter was renamed to AlphaFilter in PIXI v4+

	if (typeof WindowLayer !== 'undefined' && !WindowLayer.voidFilter) {
		try {
			WindowLayer.voidFilter = new PIXI.filters.AlphaFilter();
		} catch(e) {
			WindowLayer.voidFilter = new PIXI.Filter();
		}
	}
	if (typeof Sprite !== 'undefined' && !Sprite.voidFilter) {
		try {
			Sprite.voidFilter = new PIXI.filters.AlphaFilter();
		} catch(e) {
			Sprite.voidFilter = new PIXI.Filter();
		}
	}
	...
	Yanfly.updateResolution = function() {
		try {
			const os = require("os");
			if(os && os.platform && os.platform() !== "darwin") {return _old_yanfly_updateResolution.call(this);}
			...
			// Browser environment - skip nw.js specific code
			return _old_yanfly_updateResolution.call(this);
		}
	}
	...
	Yanfly.moveToCenter = function() {
		...
		try {
			const os = require("os");
			if(os && os.platform && os.platform() !== "darwin") {window.moveTo(x,y)}
			else {
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
			...
			// Browser environment - just use window.moveTo
			window.moveTo(x,y);
		}
	}
	...
	Scene_Boot = class extends Scene_Boot {
		...
		start() {
			...
			// Skip Steam detection for browser compatibility
		...
		determineOS() {
			try {
				...
				if(os && os.platform) {
					...
					// Browser environment - detect from navigator
					Utils.setMac(navigator.platform.includes('Mac'));
				}
			} catch(e) {
				// Browser environment - detect from navigator
				Utils.setMac(navigator.platform.includes('Mac'));
		...
		initNWScreen() {
			try {
				const nw = require("nw.gui");
				if(nw && nw.Screen && nw.Screen.Init) {
					nw.Screen.Init();
				...
				// Browser environment - skip nw.js screen initialization
				console.log('Running in browser mode, skipping nw.Screen initialization');
			}
		}
	}
	...
	Graphics = class extends Graphics {
		...
		static _createCanvas() {
			...
			this._overlayFix.style.pointerEvents = "none";
		...
		static _isFullScreen() {
			try {
				const os = require("os");
				if(os && os.platform && os.platform() !== "darwin") {return super._isFullScreen();}
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
				return !nw_window.isFullscreen;
			} catch(e) {
				// Browser environment - use standard fullscreen detection
				return super._isFullScreen();
			}
		}
		...
		static _requestFullScreen() {
			try {
				const os = require("os");
				if(os && os.platform && os.platform() !== "darwin") {return super._requestFullScreen();}
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
				return nw_window.enterFullscreen();
			} catch(e) {
				// Browser environment - use standard fullscreen
				return super._requestFullScreen();
			}
		}
		...
		static _cancelFullScreen() {
			try {
				const os = require("os");
				if(os && os.platform && os.platform() !== "darwin") {return super._cancelFullScreen();}
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
				nw_window.leaveFullscreen();
			} catch(e) {
				// Browser environment - use standard fullscreen
				return super._cancelFullScreen();
			}
		}
	...
	SceneManager = class extends SceneManager {
		...
		static requestUpdate() {
			if (!this.ticker) {
				...
				this.ticker.start();
				// Make On Minimize Event (Node.js/nw.js only)
				try {
					const nw = require("nw.gui");
					...
					if(os && os.platform && os.platform() === "darwin") {
						win.on("blur", () => {
							this._minimizeHandler = setInterval(() => {
								if(WebAudio._masterVolume <= 0) {clearInterval(this._minimizeHandler)}
								this.updateWebAudio();
					...
					// Browser environment - no nw.js available
					console.log('Running in browser mode, skipping nw.js window events');
				}
				return;
			}
			else {
	...
	DataManager = class extends DataManager {
		...
		static forceWriteToFile(world = 0) {
			...
			// Cache TITLEDATA in browser memory so it doesn't require file re-read
			window._cachedTitleData = String(world);
		}
...
(function($) {
	...
	Permanent_Manager = class {
		...
		static dataFolder() {
			// In browser, fallback cleanly to standard local paths
			return '';
		}
		...
		static load() {
			...
			const targetPath = "CUTSCENE.json";
			...
			try {
				if (fs.existsSync(targetPath)) {
					...
					console.warn('[Permanent_Manager] CUTSCENE.json not found, initializing empty.');
				}
			} catch (err) {
				console.error('[Permanent_Manager] Failed to load CUTSCENE.json:', err);
				this._cutsceneData = {};
			}
		}
		...
		static save() {
			...
			try {
				...
				const targetPath = pp.join(folder, "CUTSCENE.json");
...
(function($) {
	...
	Graphics = class extends Graphics {
		...
		static tickStart() {
			if(this._fpsMeter) {
				// The custom Omori counter uses startTick(), while the browser
				// FPSMeter/RPG Maker fallback exposes tickStart(). Support both
				// implementations so the frame loop cannot crash at boot.
				if (typeof this._fpsMeter.startTick === 'function') {
					this._fpsMeter.startTick();
				} else if (typeof this._fpsMeter.tickStart === 'function') {
		...
		static tickEnd() {
			if(this._fpsMeter) {
				// The custom counter uses endTick(); the standard FPSMeter uses
				// tick(). Keep either implementation compatible with the loop.
				if (typeof this._fpsMeter.endTick === 'function') {
					this._fpsMeter.endTick();
				} else if (typeof this._fpsMeter.tick === 'function') {
					this._fpsMeter.tick();
				}
			}
		}
...
(function($) {
	...
	DataManager = class extends DataManager {
		...
		static loadDatabase() {
			...
			if(window['$externalNotesData'] === undefined) {
				var notesText = window.ZipLoader && window.ZipLoader.getText
					? window.ZipLoader.getText('data/Notes.yaml') : null;
				if (notesText !== null) {
					window['$externalNotesData'] = yaml.load(notesText);
				} else {
					var notesXhr = new XMLHttpRequest();
					notesXhr.open('GET', 'data/Notes.yaml', false);
					notesXhr.overrideMimeType('text/plain');
					notesXhr.send();
					if(notesXhr.status < 400 && notesXhr.responseText) {
						window['$externalNotesData'] = yaml.load(notesXhr.responseText);
					}
				}
			}

			if(window["$dataQuests"] === undefined) {
				var questsText = window.ZipLoader && window.ZipLoader.getText
					? window.ZipLoader.getText('data/Quests.yaml') : null;
				if (questsText !== null) {
					window["$dataQuests"] = yaml.load(questsText);
				} else {
					var questsXhr = new XMLHttpRequest();
					questsXhr.open('GET', 'data/Quests.yaml', false);
					questsXhr.overrideMimeType('text/plain');
					questsXhr.send();
					if(questsXhr.status < 400 && questsXhr.responseText) {
						window["$dataQuests"] = yaml.load(questsXhr.responseText);
					}
				}
			}
		...
		static loadMapData(mapId) {
			...
			if(mapId > 0) {
				...
				xhr.onload = function() {
					if(xhr.status < 400) {
						window["$dataMap"] = JSON.parse(xhr.responseText);
		...
		static loadTiledMapData(mapId) {
			...
			xhr.onload = function() {
				if(xhr.status < 400) {
					...
					console.error('Failed to load tiled map');
					Graphics.printLoadingError(url);
					SceneManager.stop();
				}
			};
	...
	LanguageManager = class extends LanguageManager {
		...
		static loadLanguageFiles(language) {
			...
			// extracted into the virtual filesystem at init time.
			...
			console.log('Loading ' + yamlFiles.length + ' language files for ' + language + ' from filesystem');
			...
			for(var i = 0; i < yamlFiles.length; i++) {
				...
				try {
					...
					console.warn('Failed to load language file:', file, e.message);
				}
			}
		}
	}
	...
	//		//###############################################################################
		...
		static initAtlasData() {
			...
			if(window["$atlasData"] === undefined) {
				const yaml = require('./js/libs/js-yaml-master');
				var atlasText = window.ZipLoader && window.ZipLoader.getText
					? window.ZipLoader.getText('data/Atlas.yaml') : null;
				if (atlasText !== null) {
					window['$atlasData'] = yaml.load(atlasText);
					return;
				}
				// Keep the original NW.js path for the desktop build only.
				var xhr = new XMLHttpRequest();
				xhr.open('GET', 'data/Atlas.yaml', false);
				xhr.overrideMimeType('text/plain');
				xhr.send();
				if(xhr.status < 400 && xhr.responseText) {
					window['$atlasData'] = yaml.load(xhr.responseText);
				}
			}
		}
	}
	...
	if (window.ZipLoader && window.ZipLoader.refreshHooks) {
		window.ZipLoader.refreshHooks();
	}
...
(function($) {
	...
	DataManager = class extends DataManager {
		...
		static loadSavefileInfo(savefileId) {
			var globalInfo = this.loadGlobalInfo();
			var info = (globalInfo && globalInfo[savefileId]) ? globalInfo[savefileId] : this._savefileInfoFromSlot(savefileId);
			if (info) {
				// Work on a copy when the info came from the synthesized slot cache
				// so the cached object is never mutated by language conversion.
				if (!(globalInfo && globalInfo[savefileId])) {
					var copy = {};
					for (var key in info) { copy[key] = info[key]; }
					if (copy.actorData) {
						copy.actorData = {
							name: copy.actorData.name,
							level: copy.actorData.level,
							faceName: copy.actorData.faceName,
							faceIndex: copy.actorData.faceIndex
						};
					}
					info = copy;
				}
				if (info.actorData && info.actorData.name !== undefined) {
					info.actorData.name = this.convertNameToLanguage('actor', info.actorData.name);
					if(!isTextInCurrentLanguage(info.actorData.name)) {
						const lang = LanguageManager.defaultLanguage();
						info.actorData.name = LanguageManager._data[lang]["text"]["XX_BLUE"]["Omori_Name_Input"]["defaultname"] // Assuming this is Sunny as all the other characters that could save
		...
		static isThisGameFile(savefileId) {
			...
			if (globalInfo && globalInfo[savefileId] &&
					...
					(globalInfo[savefileId].globalId === this._globalId &&
					 globalInfo[savefileId].title === $dataSystem.gameTitle))) {
				return true;
			}
			// Browser fallback: treat an uploaded slot save as a valid game
			// file whenever it exists and parses into real save contents. This
			// also covers a mismatched/foreign global.rpgsave that fails the
			// title check above.
			return !!this._savefileInfoFromSlot(savefileId);
		}
		...
		static latestSavefileId() {
			...
			for (var i = 1; i <= this.maxSavefiles(); i++) {
				var info = (globalInfo && globalInfo[i]) ? globalInfo[i] : this._savefileInfoFromSlot(i);
				if (info && this.isThisGameFile(i) && info.timestamp > timestamp) {
		...
		// Builds a makeSavefileInfo()-style object straight from the raw save
		// contents of a slot, so uploaded .rpgsave files work even when the
		// accompanying global.rpgsave (RPG Global) is missing.
		static _savefileInfoFromSlot(savefileId) {
			try {
				if (!StorageManager.exists(savefileId)) { return null; }
				var raw = StorageManager.load(savefileId);
				if (!raw) { return null; }
				if (this._slotInfoCache && this._slotInfoCache[savefileId] &&
						this._slotInfoCache[savefileId].raw === raw) {
					return this._slotInfoCache[savefileId].info;
				}
				var contents = JsonEx.parse(raw);
				if (!contents || !contents.system) { return null; }

				var info = {};
				info.globalId = this._globalId;
				info.title = (typeof $dataSystem !== 'undefined' && $dataSystem) ? $dataSystem.gameTitle : 'OMORI';
				info.timestamp = Date.now();

				// Playtime (stored in ms by the playtime manager)
				var playtimeMs = Number(contents.system._playtime) || 0;
				var totalSec = Math.floor(playtimeMs / 1000);
				var hour = Math.floor(totalSec / 3600);
				var min = Math.floor(totalSec % 3600 / 60);
				var sec = Math.floor(totalSec % 60);
				info.playtime = hour.padZero(2) + ':' + min.padZero(2) + ':' + sec.padZero(2);

				// Leader actor (party._actors holds actor ids)
				var leaderId = contents.party && contents.party._actors && contents.party._actors.length ? contents.party._actors[0] : null;
				var actor = null;
				if (leaderId && contents.actors && contents.actors._data) {
					actor = contents.actors._data[leaderId];
				}
				if (actor) {
					info.actorData = {
						name: actor._name || '',
						level: Number(actor._level) || 1,
						faceName: this._faceNameForActor(Number(actor._actorId) || leaderId),
						faceIndex: 0
					};
				} else {
					info.actorData = { name: '', level: 1, faceName: 'default_face_image_here', faceIndex: 0 };
				}

				// Chapter is game variable 23
				if (contents.variables && contents.variables._data) {
					info.chapter = contents.variables._data[23];
				}

				// Location from map id
				var mapId = contents.map ? contents.map._mapId : 0;
				info.location = this._mapDisplayName(mapId);

				this._slotInfoCache = this._slotInfoCache || {};
				this._slotInfoCache[savefileId] = { raw: raw, info: info };
				return info;
			} catch (e) {
				return null;
			}
		...
		static _faceNameForActor(actorId) {
			switch (actorId) {
				case 1: return "01_OMORI_BATTLE";
				case 2: return "02_AUBREY_BATTLE";
				case 3: return "03_KEL_BATTLE";
				case 4: return "04_HERO_BATTLE";
				case 8: return "01_FA_OMORI_BATTLE";
				case 9: return "02_FA_AUBREY_BATTLE";
				case 10: return "03_FA_KEL_BATTLE";
				case 11: return "04_FA_HERO_BATTLE";
				default: return "default_face_image_here";
			}
		}

		static _mapDisplayName(mapId) {
			try {
				var loc = LanguageManager.getMessageData("XX_BLUE.Map_Display_Names");
				if (loc && loc[mapId]) { return loc[mapId]; }
			} catch (e) {}
