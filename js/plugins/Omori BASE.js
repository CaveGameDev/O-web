...
Bitmap.prototype.drawKeyIcon = function(key, x, y, type = this.defaultInputIcons(), side = 0) {
  ...
  if (!rect) { rect = {x:0,y:0,width:24,height:24}; }
  // Determine key type;
  let rectX = rect.x;
  if(!!rectsFound) {
...
ConfigManager.applyData = function(config) {
  ...
  // Keep the browser game at its native logical resolution. The canvas is
  // stretched by Graphics when needed; changing the logical box breaks the
  // original 640x480 coordinates used by windows, pictures, and buttons.
  Yanfly.Param.ScreenWidth = 640;
  Yanfly.Param.ScreenHeight = 480;
  SceneManager._screenWidth = 640;
  SceneManager._screenHeight = 480;
  SceneManager._boxWidth = 640;
  SceneManager._boxHeight = 480;
  // ConfigManager.load runs after SceneManager.initGraphics. Keep an already
  // created canvas and its logical box in sync as well.
  if (typeof Graphics !== 'undefined' && Graphics._canvas) {
    Graphics._width = 640;
    Graphics._height = 480;
    Graphics._boxWidth = 640;
    Graphics._boxHeight = 480;
    Graphics._updateAllElements();
  }
  config.screenResolution = 0;
      ...
      switch(command.code){
        ...
        case 356:
          ...
          if (commandName === 'ShowMessage') {
            ...
            // Missing localized messages are valid for older saves; there is
            // nothing to preload when the lookup did not resolve.
            if (!data) { continue; }
...
Scene_AtlasMaker.prototype.resizeWindowToScreen = function() {
  // The atlas maker is an in-game tool; it must not replace the game's
  // logical resolution with the physical browser/screen resolution.
  Yanfly.Param.ScreenWidth = 640;
  Yanfly.Param.ScreenHeight = 480;
  SceneManager._screenWidth = 640;
  SceneManager._screenHeight = 480;
