...
Scene_Boot.prototype.start = function() {
  ...
  // main.js and SceneManager.run already gate the game behind the explicit
  // Launch button. Keep this scene hook focused on creating the first scene.
  var self = this;
  var doBoot = function() {
...
Scene_OmoriTitleScreen.prototype.initialize = function() {
  ...
  // Try cached value first, then file reads as fallback
  let tryTitleData = window._cachedTitleData;
  if (!tryTitleData) { tryTitleData = DataManager.readFromFile("TITLEDATA"); }
  if (!tryTitleData) { tryTitleData = DataManager.readFromFile("File"); }
  ...
  // Scale factors for 816x624 (from original 640x480)
...
Scene_OmoriTitleScreen.prototype.refreshCommandHints = function() {
  ...
  let langData = LanguageManager.languageData();
  let confirm = langData && langData.text && langData.text.System && langData.text.System.plugins && langData.text.System.plugins.optionsMenu && langData.text.System.plugins.optionsMenu.buttonHints ? langData.text.System.plugins.optionsMenu.buttonHints["confirm"] : "OK";
  let cancel = langData && langData.text && langData.text.System && langData.text.System.plugins && langData.text.System.plugins.optionsMenu && langData.text.System.plugins.optionsMenu.buttonHints ? langData.text.System.plugins.optionsMenu.buttonHints["cancel"] : "Cancel";
  let iconSize = 24;
  let paddingY = 4;
  let messageData = LanguageManager.getMessageData("XX_BLUE.Scene_OmoriTitleScreen");
  this._commandHints.bitmap.fontSize = messageData && messageData.commandHints_fontsize ? messageData.commandHints_fontsize : 20;
  this._commandHints.bitmap.drawInputIcon("ok", 0, paddingY);
  let loc_position = messageData && messageData.commandHints_confirm_position ? messageData.commandHints_confirm_position : [0, 0];
  let confirmX = 0, confirmY = 0;
  try {
    confirmX = typeof loc_position[0] === 'string' ? eval(loc_position[0]) : loc_position[0];
    confirmY = typeof loc_position[1] === 'string' ? eval(loc_position[1]) : loc_position[1];
  } catch(e) {
    confirmX = 0; confirmY = 0;
  }
  this._commandHints.bitmap.drawText(confirm, confirmX, confirmY, this._commandHints.bitmap.width, 16, "left");
  this._commandHints.bitmap.drawInputIcon("escape", iconSize + 15 + this._commandHints.bitmap.measureTextWidth(confirm), paddingY);
  loc_position = messageData && messageData.commandHints_cancel_position ? messageData.commandHints_cancel_position : [0, 0];
  let cancelX = 0, cancelY = 0;
  try {
    cancelX = typeof loc_position[0] === 'string' ? eval(loc_position[0]) : loc_position[0];
    cancelY = typeof loc_position[1] === 'string' ? eval(loc_position[1]) : loc_position[1];
  } catch(e) {
    cancelX = 0; cancelY = 0;
  }
  this._commandHints.bitmap.drawText(cancel, cancelX, cancelY, this._commandHints.bitmap.width, 16, "left");
}
  ...
  // Create Omori Sprite
	if (this._worldType != 448) {
      ...
      this._omoriSprite = new Sprite(ImageManager.loadPicture('OMO_RS'));
		...
		this._omoriSprite.scale.set(this._titleScaleX, this._titleScaleY);
...
Scene_OmoriTitleScreen.prototype.createTitleSprites = function() {
  ...
  this._titleTextContainerSprite.x = 167 * this._titleScaleX;
  this._titleTextContainerSprite.y = 130 * this._titleScaleY;
  this._titleTextContainerSprite.scale.set(this._titleScaleX, this._titleScaleY);
...
Window_OmoTitleScreenBox.prototype.refresh = function() {
  ...
  // Draw Text - manually center to avoid font-loading edge cases
  ...
  var centerX = Math.max(0, Math.floor((this.contents.width - textW) / 2));
  // Nudge text up 3px from language-defined position to fix "too low" title screen buttons
  var textY = (loc_position && loc_position[1] !== undefined) ? loc_position[1] - 3 : -3;
  this.drawText(this._text, centerX, textY, this.contents.width, 'left');
};
