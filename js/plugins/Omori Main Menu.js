...
Window_Gold.prototype.refresh = function() {
  ...
  this.contents.clear();
  const goldData = LanguageManager.getMessageData("XX_BLUE.Window_Gold");
  if (goldData && goldData.drawCurrencyValue_position) {
    const loc_position = goldData.drawCurrencyValue_position;
    var posX = x;
    try { posX = eval(loc_position[0]); } catch(e) { posX = x; }
    this.drawCurrencyValue(this.value(), this.currencyUnit(), posX, loc_position[1], width);
  } else {
